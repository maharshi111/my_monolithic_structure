const CollegeService = require("../../db/services/CollegeService");
...
exports.login = async (req) => {
    ...
    let user = await CollegeService.findByEmail(email).withPassword().withUserType().withBasicInfo().execute();
...
exports.logout = async (req) => {
    ...
    CollegeService.removeAuth(req.user[TableFields.ID], headerToken);
};
...
exports.forgotPassword = async (req) => {
    ...
    let {code, name, email} = await CollegeService.getResetPasswordToken(providedEmail);
...
exports.resetPassword = async (req) => {
    ...
    let user = await CollegeService.resetPassword(providedEmail, code, newPassword);
    let token = await createAndStoreAuthToken(user);
    return {
        user: await CollegeService.getUserById(user[TableFields.ID])
        .withPassword()
        .withUserType()
        .withBasicInfo()
        .execute(),
...
exports.changePassword = async (req) => {
    ...
    let user = await CollegeService.getUserById(req.user[TableFields.ID]).withPassword().withId().execute();
    ...
    if (user && (await user.isValidAuth(oldPassword))) {
        ...
        await CollegeService.updatePasswordAndInsertLatestToken(user, newPassword, token);
        return {token};
...
async function createAndStoreAuthToken(userObj) {
    ...
    await CollegeService.saveAuthToken(userObj[TableFields.ID], token);
    return token;
}