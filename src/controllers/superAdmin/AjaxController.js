...
exports.postAjaxAddCeo = async(req,res,next) =>{
    ...
    let org = await OrganisationService.findOneOrgByEmail(ceoEmail).withOrgCeo().execute();
    if(!org){
...
exports.postAjaxAddAdmin =  async(req,res,next) =>{
    ...
    let emp = await EmployeeService.findOneEmpByEmail(adminEmail).withEmployeeBasicInfo().execute();
    if(!emp){
    ...
    const ceoOrg = await OrganisationService.findOneOrgByEmail(ceoEmail).withOrgCeo().execute();
    if (!ceoOrg) {
    ...
    let org = await OrganisationService.findByIdOrgId(orgId).withOrgCeo().execute();
