...
exports.postAddOrganisation = async (req, res, next) => {
    ...
    console.log(typeof(reqBody[TableFields.subscriptionStart]));
      ...
      const organisation = {
        ...
        [TableFields.employeeStrength]: Number(reqBody[TableFields.empStrength].trim()),
        [TableFields.startDateOfSubscription]:  new Date(reqBody[TableFields.subscriptionStart].trim()).toISOString(),
        [TableFields.subscriptionPeriod]: Number(reqBody[TableFields.subscriptionPeriod].trim()),
        [TableFields.subscriptionCharge]: Number(reqBody[TableFields.charge].trim()),
  ...
  exports.postEditOrganisation = async (req, res) => {
    ...
    const result = Util.subscriptionStartInvalidEdit(reqBody[TableFields.subscriptionStart].trim());
    console.log("this is start date",reqBody[TableFields.subscriptionStart]);
    if (!result.success) {
      throw new ValidationError(result.msg);
    }
    ...
    let pastOrgCeo = await OrganisationService.findByIdOrgId(orgId).withOrgCeo().withUniqueId().execute();
    ...
    if(pastOrgCeo[TableFields.orgCEO][TableFields.email]!== reqBody[TableFields.ceoEmail].trim().toLowerCase()){
