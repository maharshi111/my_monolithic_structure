...
exports.postAddEditAdmin = async(req,res,next)=>{
    ...
    if(!Util.ValidationMsgsLength(reqBody[TableFields.adminEmail],30,0,[TableFields.adminEmail]).flag){
        throw new ValidationError(Util.ValidationMsgsLength(reqBody[TableFields.adminEmail],30,0,[TableFields.adminEmail]).message);
    }
    let org = await OrganisationService.findOneOrgByEmail(reqBody[TableFields.ceoEmail]).withOrgCeo().withOrgAdmin().execute();
    if(!org){
        throw new ValidationError(ValidationMsgs.CeoEmalExist);
    }
    let employee = await EmployeeService.findOneEmpByEmail(reqBody[TableFields.adminEmail].trim()).withEmployeeBasicInfo().execute();
    if(!employee){
